...
class TecnicoBD:
    def __init__(self, win):
        ...
        self.btnGExcel = Button(win, text='Gerar Excel', command=self.fGerarExcel).grid(row=6, column=5, padx=5, pady=20)
        ...
        self.treeTecnicos.column("Email", minwidth=0, width=120)
...
#----------------------------------------------------------------------------------------------------------------------
    def fCadastrarTecnico(self):
        try:
            ...
            self.objBD.inserirDados(nome, cpf, celular, turno, equipe, tipo_user, login, email, self.hash)
...
#-----------------------------------------------------------------------------------------------------------------------
    def fAtualizarTecnico(self):
        try:
            ...
            self.hash = hashlib.md5(self.string)
            self.hash = self.hash.hexdigest()
            self.objBD.atualizarDados(codigo, nome, cpf, celular, turno, equipe, tipo_user, login, email, self.hash)
            # Recarrega dados na tela
            self.treeTecnicos.delete(*self.treeTecnicos.get_children())
            self.carregarDadosIniciais()
            self.fLimparTela()
