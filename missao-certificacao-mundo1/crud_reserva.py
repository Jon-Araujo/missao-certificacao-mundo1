...
# Inserir reserva
#----------------------------------------------------------------------------------------------------------------------
    def inserirDados(self, ferramenta_id, descricao, data, horar, horad, tecnico_id, mail):
        try:
            if(self.verificaReserva(ferramenta_id, data, horar, horad)):
                self.abrirConexao()
                cursor = self.connection.cursor()
                sql_insert_query = "INSERT INTO reserva(ferramenta_id, descricao, data, horar, horad, tecnico_id, mail) " \
                                   "VALUES (:ferramenta_id, :descricao, :data, :horar, :horad, :tecnico_id, :mail)"
                record_to_insert = {'ferramenta_id':ferramenta_id, 'descricao':descricao, 'data':data, 'horar':horar,
                                    'horad':horad, 'tecnico_id':tecnico_id, 'mail':mail}
...
# Atualizar reserva
#----------------------------------------------------------------------------------------------------------------------
    def atualizarDados(self, codigo, ferramenta_id, descricao, data, horar, horad, tecnico_id, mail):
        try:
            if (self.verificaReserva(ferramenta_id, data, horar, horad)):
                self.abrirConexao()
                cursor = self.connection.cursor()
                sql_update_query = "UPDATE reserva SET ferramenta_id=:ferramenta_id, descricao=:descricao, data=:data," \
                                   " horar=:horar, horad=:horad, tecnico_id=:tecnico_id, mail=:mail WHERE id=:id"
...
# ----------------------------------------------------------------------------------------------------------------------
    def comboEmail(self):
        try:
            self.abrirConexao()
            cursor = self.connection.cursor()
            sql_select_query = "SELECT id, email FROM tecnico ORDER BY id"
            cursor.execute(sql_select_query)
            registros = dict(cursor.fetchall())
        except (Exception, sqlite3.Error) as error:
