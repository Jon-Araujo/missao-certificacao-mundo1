...
class ReservaBD:
    def __init__(self, win):
        ...
        self.lblEmail = Label(win, text='Email').grid(row=5, column=0, padx=5, pady=5)
        self.dicEmail = self.objBD.comboEmail()
        self.Email = StringVar()
        self.comboEmail = ttk.Combobox(win, values=list(self.dicEmail.values()), textvariable=self.Email)
        self.comboEmail.grid(row=5, column=1, padx=5, pady=5)
        ...
        self.dadosColunas = ("Codigo", "Ferramenta", "Descricao", 'Data', 'Retirada', 'Devolucao', 'Tecnico', 'Email')
        ...
        self.treeReserva.heading("Email", text="Email")
        ...
        self.treeReserva.column("Email", minwidth=0, width=120)
...
# ----------------------------------------------------------------------------------------------------------------------
    def apresentarRegistrosSelecionados(self, event):
        ...
        for selection in self.treeReserva.selection():
            ...
            codigo, ferramenta_id, descricao, data, horar, horad, tecnico_id, mail = item["values"][0:8]
...
# ----------------------------------------------------------------------------------------------------------------------
    def fLerCampos(self):
        ...
        return codigo, ferramenta_id, descricao, data, horar, horad, tecnico_id, mail
#----------------------------------------------------------------------------------------------------------------------
...
#----------------------------------------------------------------------------------------------------------------------
    def fCadastrarReserva(self):
        try:
            ...
            tecnico_id = self.Tecnico.get()
            mail = self.Email.get()
            self.objBD.inserirDados(ferramenta_id, descricao, data, horar, horad, tecnico_id, mail)
            self.treeReserva.delete(*self.treeReserva.get_children())
            self.carregarDadosIniciais()
            self.fLimparTela()
            email_reserva(tecnico_id, ferramenta_id, data, horar, descricao, horad, mail)
        except:
...
#-----------------------------------------------------------------------------------------------------------------------
    def fExcluirReserva(self):
        try:
            codigo, ferramenta_id, descricao, data, horar, horad, tecnico_id, mail = self.fLerCampos()
...
#-----------------------------------------------------------------------------------------------------------------------
    def fAtualizarReserva(self):
        try:
            codigo, ferramenta_id, descricao, data, horar, horad, tecnico_id, mail = self.fLerCampos()
            self.objBD.atualizarDados(codigo, ferramenta_id, descricao, data, horar, horad, tecnico_id, mail)
            # Recarrega dados na tela
            self.treeReserva.delete(*self.treeReserva.get_children())
            self.carregarDadosIniciais()
            self.fLimparTela()
